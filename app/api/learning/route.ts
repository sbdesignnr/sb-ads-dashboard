...
export async function POST(req: NextRequest) {
  ...
  const meta = await lookupByTitle(title, body.author?.trim() || undefined).catch(() => null);
  ...
  const dupe = (await prisma.learningBook.findMany({ select: { title: true } })).find(
    (b) => bookKey(b.title) === bookKey(finalTitle),
  );
  if (dupe) return NextResponse.json({ error: "duplicate", message: "Túto knihu už v knižnici máš." }, { status: 409 });
  ...
  if (!desc) return NextResponse.json({ error: "describe_failed" }, { status: 500 });

  // New manual books go to the front of the "next up" shelf.
  const minPriority = (await prisma.learningBook.aggregate({ _min: { priority: true } }))._min.priority ?? 1;
  ...
  return NextResponse.json({ book: serializeBook(book), foundCover: Boolean(meta?.coverUrl) });
}
