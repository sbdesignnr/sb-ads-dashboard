...
const WIN1250: Record<string, number> = {
  á: 0xe1, ä: 0xe4, č: 0xe8, ď: 0xef, é: 0xe9, í: 0xed, ĺ: 0xe5, ľ: 0xba,
  ň: 0xf2, ó: 0xf3, ô: 0xf4, ŕ: 0xe0, š: 0x9a, ť: 0x9d, ú: 0xfa, ý: 0xfd, ž: 0x9e,
  Á: 0xc1, Ä: 0xc4, Č: 0xc8, Ď: 0xcf, É: 0xc9, Í: 0xcd, Ĺ: 0xc5, Ľ: 0xa5,
  Ň: 0xd2, Ó: 0xd3, Ô: 0xd4, Ŕ: 0xc0, Š: 0x8a, Ť: 0x8d, Ú: 0xda, Ý: 0xdd, Ž: 0x8e,
};
...
function encodeWin1250(input: string): string {
  ...
  for (const ch of input) {
    ...
    else if (WIN1250[ch] !== undefined) out += `%${WIN1250[ch].toString(16).toUpperCase()}`;
...
function parseResults(html: string, limit: number): OrsrCompany[] {
  ...
  $('a[href*="vypis.asp"]').each((_, a) => {
    ...
    if (name && !out.find((r) => r.id === m[1])) out.push({ name, id: m[1], sid: m[2] });
  });
  return out.slice(0, limit);
}

/** Search the register by company name / keyword. */
export async function searchCompanies(query: string, limit = 20): Promise<OrsrCompany[]> {
...
export async function searchByIco(ico: string): Promise<OrsrCompany | null> {
  ...
  const html = await getWin1250(`https://www.orsr.sk/hladaj_ico.asp?ICO=${clean}&SID=0&lan=sk`);
...
function extractCity(address: string | null): string | null {
  ...
  const m = address.match(/\d+(?:\/\d+)?\s+([A-ZÁ-Ž][a-zá-žäôňčďĺľŕšťžýíéóú]+)/);
  return m ? m[1] : null;
}

/** Aktuálny výpis → IČO, sídlo, city, konateľ name + position. */
export async function getCompanyDetail(id: string, sid: string): Promise<OrsrDetail> {
  const html = await getWin1250(`https://www.orsr.sk/vypis.asp?ID=${id}&SID=${sid}&P=0`);
  const text = cheerio.load(html).text().replace(/\s+/g, " ");

  const ico = text.match(/IČO:\s*([\d ]+?)\s*\(od:/)?.[1]?.replace(/\s/g, "") ?? null;
  const address = text.match(/Sídlo:\s*(.+?)\s*\(od:/)?.[1]?.trim() ?? null;

  let ownerName: string | null = null;
  let ownerPosition: string | null = null;
  const org = text.match(/Štatutárny orgán:\s*([^(]+?)\s*\(od:[^)]*\)\s*(.+?)(?:Spoločníci|Základné imanie|Konanie|$)/);
  if (org) {
    ownerPosition = org[1].replace(/\s+/g, " ").trim() || null;
